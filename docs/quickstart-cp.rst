...
#. Navigate to examples directory (``/examples/cp-all-in-one/``) and start the |zk| and Kafka containers in detached
   mode (``-d``). 

   .. sourcecode:: bash

       cd <path-to-cp-docker-images>/examples/cp-all-in-one/
       docker-compose up -d
...
#. Publish some data to the ``foo`` topic. This command uses the built-in Kafka Console Producer to produce 42 simple
   ...
   .. sourcecode:: bash
    ...
    $ docker-compose exec broker  \
      ...
      broker:9092 --topic foo && echo 'Produced 42 messages.'"
...
#. Read back the message using the built-in Console consumer:
   ...
   .. sourcecode:: bash
    ...
    $ docker-compose exec broker  \
      kafka-console-consumer --bootstrap-server broker:9092 --topic foo \
